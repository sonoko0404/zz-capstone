...
function normalizeSession(value: UserSession): UserSession {
  return {
    email: value.email,
...
export async function loginWithEmail(email: string): Promise<UserSession> {
  ...
  if (apiUrl) {
    ...
    const session = normalizeSession((await response.json()) as UserSession)
    saveSession(session)
    return session
  }
